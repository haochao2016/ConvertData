...
object ReaderTAQWithCondition5 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("ReaderTAQWithCondition5")
      .getOrCreate()
...
//    val frame = spark.sql("select time, ofr from taq where date = to_date('2007-08-01') and symbol= 'IBM' ")
...
//    val frame = spark.sql("select time, ofr, bid, symbol  where date=to_date('2007-08-03') ")
...
//         "  order by Date desc , SYMBOL  ")
    ...
    val frame = spark.sql("select BID, BIDSIZ, DATE, SYMBOL from TAQ")
...
//    val framePer = frame.cache()
    val framePer = frame.persist(StorageLevel.MEMORY_ONLY)
    frame.show()

    framePer.createTempView("frame")
    val perRes =  spark.sql("select sum(BID * BIDSIZ)/sum(BIDSIZ) as vwab from frame group by DATE, SYMBOL having sum(BIDSIZ) > 0 " +
      "  order by DATE desc ,SYMBOL ")
    perRes.show()
    perRes.printSchema()
    ...
    spark.sql("select sum(BID * BIDSIZ)/sum(BIDSIZ) as vwab from frame group by DATE, SYMBOL having sum(BIDSIZ) > 0 " +
      "  order by DATE desc ,SYMBOL ").show()
