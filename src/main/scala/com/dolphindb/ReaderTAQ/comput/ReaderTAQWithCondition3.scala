...
object ReaderTAQWithCondition3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("ReaderTAQWithCondition3")
      .getOrCreate()
...
//                       "  group by minute ")
    ...
    val frame = spark.sql("select TIME, OFR from TAQ where DATE = to_date('2007-08-01') ")
...
//    frame.foreachPartition(x => println( " partition : " + TaskContext.getPartitionId()))
    ...
    val perRes = spark.sql("select minute(TIME) as minute, avg(OFR) as spread from frame   group by minute")
    perRes.show()
    perRes.printSchema()

    val beginTime = System.currentTimeMillis()
    println("起始时间   ====>   " + beginTime)
    spark.sql("select minute(TIME) as minute, avg(OFR) as spread from frame   group by minute").show()
