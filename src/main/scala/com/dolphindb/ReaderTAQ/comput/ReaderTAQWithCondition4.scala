...
//       " (max(OFR)-min(BID)) as gap from TAQ where date=to_date('2007-08-03')and symbol='IBM' and OFR>BID and BID>0 group by SYMBOL, minute" )
    ...
    val frame = spark.sql("select TIME, OFR, BID, SYMBOL from TAQ  where DATE= to_date('2007-08-03') ")
...
//    frame.foreachPartition(x => println( " partition : " + TaskContext.getPartitionId()))
    ...
    val perRes = spark.sql("select minute(TIME) as minute,(max(frame.OFR)-min(frame.BID)) as gap  from frame where frame.OFR > frame.BID and frame.BID>0 group by SYMBOL, minute")
    perRes.show()
    perRes.printSchema()
    ...
    spark.sql("select minute(TIME) as minute,(max(frame.OFR)-min(frame.BID)) as gap  from frame where frame.OFR > frame.BID and frame.BID>0 group by SYMBOL, minute").show()
