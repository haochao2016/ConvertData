...
object Linux_writeToHive {
  def main(args: Array[String]): Unit = {

    val startTime = System.currentTimeMillis()
    val spark = SparkSession.builder().appName("Linux_writeToHive")
//      .master("local[7]")
      .enableHiveSupport()
      .getOrCreate()
...
//    val hdds = Array("0817","0820","0821", "0822")
...
//    for (hdd <- hdds) {
      val dataDF = spark.read
        .option("header", "true")
        .csv(s"file:///hdd/hdd3/Spark-DolphinDB-TAQ/TAQ2007${hdd}.csv")
...
//    spark.sql(s"insert into taq partition (date= df2.date, symbol) select time,bid,ofr, bidsiz , mode, ex, mmid, ofrsiz , symbol from df2")
    spark.sql(s"from df2 df insert into taq partition (date , symbol) select df.time, df.bid,df.ofr, df.bidsiz ,df.ofrsiz , df.mode, df.ex, df.mmid,  df.date, df.symbol ")
//    spark.catalog.dropTempView(s"sparkCSV${arri}")
//    }

    val endTime = System.currentTimeMillis()
    println("===ALL import Time===  " + (endTime-startTime))
    val writer = new PrintWriter(new File("/home/chao/apps/spark/writerH1.txt"))
