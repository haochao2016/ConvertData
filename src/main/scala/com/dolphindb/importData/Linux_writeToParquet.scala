...
object Linux_writeToParquet {

  def main(args: Array[String]): Unit = {

    val startTime = System.currentTimeMillis()
    val spark = SparkSession.builder().appName("Linux_writeToParquet")
//      .master("local[3]")
      .getOrCreate()

    import spark.implicits._

    val hdds = Array("hdd3","hdd4","hdd5")
//    val hdds = Array("hdd3")
    ...
    for (hdd <- hdds) {
      val dataDF = spark.read
        .option("header", "true")
        .csv(s"file:///hdd/${hdd}/Spark-DolphinDB-TAQ/TAQ2007*")
      ...
      dataDF2.printSchema()
      println("==========SCHEMA==============")
      dataDF2.write.parquet("hdfs://cnserver2:9000/chao/dolphindb/TAQ")
    }

    val endTime = System.currentTimeMillis()
    println("===ALL import Time===  " + (endTime-startTime))
    val writer = new PrintWriter(new File("/home/chao/apps/spark"))
    writer.write("===ALL import Time===  " + (endTime-startTime))
    writer.close()
    spark.stop()
